...
> **状态：已实现（提交 `8c25cf7`）。** 本文保留为设计记录，
> 下面「实现后的偏差」列出与原始设计的实际差异。

## 实现后的偏差

| 项 | 设计 | 实际 | 原因 |
|---|---|---|---|
| 框选（原待确认 1） | 待定 | **做了** | 多选在验玩法时太实用，且实现量可控 |
| `Shift` = 抽单张、`G` = 吸附（原待确认 2） | 提议 | **照做** | — |
| 骰子统一圆角方形（原待确认 3） | 提议 | **照做** | 并按面数上色，桌上能一眼认出 D20 |
| 11 个布局槽位（原待确认 4） | 提议 | **照做** | 未出现不够用的情况 |
| 掷骰触发 | 未写明 | **轻点骰子即掷** | 掷骰是高频操作，不该逼人翻右键菜单 |
| `Alt`+滚轮旋转 | 物件系统自己抢输入 | **新增 `IWheelHandler` 接口** | 让输入路由统一分发，避免按节点顺序抢事件 |
| 框选起点 | 未考虑 | **M1 补了 `PrimaryDragStarted` 信号** | 空白拖动不会有 `PrimaryPressed`，而 `PrimaryDragged` 只给增量拿不到起点 |

另外两处实现细节值得记下来：

- **`MergeInto` 不会因生成而成堆**：只有拖放或显式调用 `GroupIntoPile` 才会。
  DemoContent 最初把三张卡生成在相邻位置，以为看起来像一堆，实际是三个重叠散件。
- **绘制次序只有一处真相**：`_drawOrder` 列表，节点顺序由 `MoveChild` 单向同步过去。
  不要反过来从节点顺序推断绘制次序。
