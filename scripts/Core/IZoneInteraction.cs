...
public interface IZoneInteraction
{
	...
	/// <summary>
	/// 该物件所属的<b>区域</b>（不分排版）；不属于任何区域时返回 <c>null</c>。
	///
	/// <b>与 <see cref="StackZoneOf"/> 的区别是"要不要按排版筛"。</b>
	/// 用户实测报过一条不一致（"在区域内对卡组进行翻转，和在桌面上的结果不一致"）：
	/// 指着牌库按 <c>F</c> 翻整摞、指着横排的手牌按 <c>F</c> 却只翻一张 ——
	/// 根因就是当时只认叠放区域。
	///
	/// 判据应该是<b>"这一组是不是一个整体"</b>（一手横排的牌与一摞牌在这一点上没有区别），
	/// 而不是"它是怎么排的"。怎么翻（整组 / 只翻顶上一张）由按键决定。
	/// </summary>
	Zone? ZoneOf(TabletopObject obj);
